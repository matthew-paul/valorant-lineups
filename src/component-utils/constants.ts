...
import SummitMap from "../resources/Maps/summit_map.png";
...
export const ABILITY_LIST: Partial<Record<number, AbilityOption[]>> = {
  2: [{ value: 1, label: "Aftershock", icon: Aftershock }],
  3: [{ value: 1, label: "Incendiary", icon: Incendiary }],
  4: [
  ...
  6: [{ value: 1, label: "Nanoswarm", icon: Nanoswarm }],
  16: [
...
export const MAP_LIST: MapOption[] = [
  ...
  { value: 12, label: "Summit", icon: SummitMap },
  { value: 10, label: "Sunset", icon: SunsetMap },
];

export const TAG_LIST: TagOption[] = [
...
export const getAgentFromId = (agentId: number): AgentOption | undefined => {
  return AGENT_LIST.find((agent) => agent.value === agentId);
};

export const getAbilityFromId = (
  agentId: number,
  abilityId: number
): AbilityOption | undefined => {
  return ABILITY_LIST[agentId]?.find(
    (ability) => ability.value === abilityId
  );
};

export const getTagsFromIds = (tagIds: readonly number[]): TagOption[] => {
  return TAG_LIST.filter((tag) => tagIds.includes(tag.value));
};

export const getImagesFromIds = (imageIds: readonly string[]): ImageTag[] => {
  return imageIds.map((image) => ({ id: image, text: image }));
};

export const getMapFromId = (mapId: number): MapOption | undefined => {
